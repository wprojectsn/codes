...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, vocab, concept_vocab):
    ...
    if config.pointer_gen:
      self.enc_input_extend_vocab, self.article_oovs, self.enc_input_concept_extend_vocab, self.concept_p, self.position, self.concept_mask = data.article2ids(article_words, vocab, concept_vocab)
  ...
  def pad_encoder_input(self, max_len, pad_id):
    ...
    if config.pointer_gen:
      c_pad_id = []
      c_pad_id.append(pad_id)
      c_pad_id.append(pad_id)
      p_pad = [0,0]
      while len(self.enc_input_extend_vocab) < max_len:
        self.enc_input_extend_vocab.append(pad_id)
        self.enc_input_concept_extend_vocab.append(c_pad_id)
        self.concept_p.append(p_pad)
        self.position.append(c_pad_id)
        self.concept_mask.append(p_pad)
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list):
    ...
    if config.pointer_gen:
      ...
      self.enc_batch_concept_extend_vocab = np.zeros((self.batch_size, max_enc_seq_len, config.concept_num), dtype=np.int32)
      self.concept_p = np.zeros((self.batch_size, max_enc_seq_len, config.concept_num), dtype=np.float32)
      self.concept_mask = np.zeros((self.batch_size, max_enc_seq_len, config.concept_num), dtype=np.float32)
      self.position = np.zeros((self.batch_size, max_enc_seq_len, config.concept_num), dtype=np.int32)
      for i, ex in enumerate(example_list):
        self.enc_batch_extend_vocab[i, :] = ex.enc_input_extend_vocab[:]
        self.enc_batch_concept_extend_vocab[i, ::] = ex.enc_input_concept_extend_vocab[:]
        self.concept_p[i, ::] = ex.concept_p[:]
        self.position[i, ::] = ex.position[:]
        self.concept_mask[i, ::] = ex.concept_mask[:]
...
class Batcher(object):
  ...
  def __init__(self, data_path, vocab, concept_vocab, mode, batch_size, single_pass):
    self._data_path = data_path
    self._vocab = vocab
    self._concept_vocab = concept_vocab
  ...
  def fill_example_queue(self):
    ...
    while True:
      ...
      example = Example(article, abstract_sentences, self._vocab, self._concept_vocab)
